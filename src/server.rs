...
#[http_component]
async fn handle_spin_leptos(req: IncomingRequest, resp_out: ResponseOutparam) {
    let mut conf = leptos::get_configuration(None).await.unwrap();
    // conf.leptos_options.output_name = "spin_leptos".to_owned();
    conf.leptos_options.output_name = "spin_leptos".into();
